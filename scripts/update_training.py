#!/usr/bin/env python3
"""
Update lessons in the D1 database with full verbatim content.
"""
...
def run_sql(sql):
    ...
    cmd = [
        'npx', 'wrangler', 'd1', 'execute', 'shortcircuits-db', '--remote',
        '--command', sql
    ]
    try:
...
# ============================================
# SMARTWATCH TRAINING MATERIAL LESSONS
# ============================================

lessons = []

# lesson-t-overview: Complete Project Overview
lessons.append({
    "id": "lesson-t-overview",
    "description": "Watch the complete project overview video to understand what you will build.",
    "content_json": [
...
# lesson-t-modular: Modular Architecture
lessons.append({
    "id": "lesson-t-modular",
    "description": "Learn how professional embedded firmware is structured into modular, maintainable layers.",
    "content_json": [
...
# lesson-t-files: File Structure: Headers and Source Files
lessons.append({
    "id": "lesson-t-files",
    "description": "Understand the separation between header files and source files in C/C++ projects.",
    "content_json": [
...
# lesson-t-layers: Project Layers
lessons.append({
    "id": "lesson-t-layers",
    "description": "Learn the five conceptual layers of well-structured embedded firmware.",
    "content_json": [
...
# lesson-t-pointers: Pointers and References
lessons.append({
    "id": "lesson-t-pointers",
    "description": "Master pointers and references for efficient embedded firmware development.",
    "content_json": [
        ...
        {"type":"bullets","content":["A constant pointer (int *const p) means the pointer cannot point to a different address, but the value it points to can change.","A pointer to constant data (const int *p) means the pointer can change, but the value it points to cannot be modified.","A constant pointer to constant data (const int *const p) means neither the pointer nor the value can change."]},
        ...
        {"type":"text","content":"Pointers can be a challenging topic when first encountered. The following video by Nicolai Nielsen provides a clear and practical overview:"},
        {"type":"youtube","content":"https://www.youtube.com/watch?v=syy-3fVicUc"}
    ]
})

# lesson-t-bits: Bit Manipulation
lessons.append({
    "id": "lesson-t-bits",
    "description": "Learn essential bit manipulation techniques for working with hardware registers.",
    "content_json": [
        ...
        {"type":"text","content":"The following video from Portfolio Courses offers a great introduction into binary numbers and bitwise operators:"},
        ...
        {"type":"text","content":"In the case of the PCF8563 RTC, the seconds register stores valid time data in bits 0 through 6, while bit 7 indicates clock integrity. When reading this register, firmware must apply a mask to remove the flag bit before converting the remaining value."},
        ...
        {"type":"text","content":"More complex operations involve updating a bit field, where multiple adjacent bits represent a single value. In these cases, the existing bits are first cleared using a mask, and the new value is then shifted into position and ORed into the cleared space."},
...
# lesson-t-i2c: I2C Communication
lessons.append({
    "id": "lesson-t-i2c",
    "description": "Understand I2C protocol fundamentals and driver development.",
    "content_json": [
        ...
        {"type":"text","content":"Every I2C transaction follows a defined sequence: a START condition, transmission of the slave address with a read/write bit, an acknowledgment (ACK) from the receiver, one or more data bytes with acknowledgments after each byte, and finally a STOP condition."},
        ...
        {"type":"text","content":"From the schematic, you must identify which pins are connected to SDA and SCL, as well as which I2C bus instance is being used (for example, I2C_NUM_0 or I2C_NUM_1). From the datasheet, you must determine the device's 7-bit I2C address, understand the register map that defines how data is stored, and verify the maximum supported clock speed."},
        {"type":"subheading","content":"Driver Architecture"},
        {"type":"text","content":"Well-structured I2C drivers are typically written in layers. The lowest layer is the bus layer, which handles raw I2C transactions such as initializing the bus, sending bytes, and reading bytes."},
        {"type":"text","content":"Above the bus layer sits the driver logic, which interprets device-specific register layouts and data formats. At the top is the API layer, which exposes simple, readable functions that the rest of the application can use without needing to understand low-level I2C details."},
        ...
        {"type":"bullets","content":["Verify the slave address matches the datasheet (e.g., 0x19, 0x68, or 0x51)","Check that both SDA and SCL have proper pull-up resistors","Write a minimal test routine that probes for device acknowledgments","In multi-tasking systems, protect I2C transactions with a mutex","Most I2C issues stem from incorrect wiring, timing, or register assumptions"]},
        {"type":"text","content":"This video from Rohde & Schwarz offers a great overview of the I2C protocol:"},
        {"type":"youtube","content":"https://www.youtube.com/watch?v=CAvawEcxoPU"}
    ]
})

# lesson-t-oop: Object-Oriented Programming Basics
lessons.append({
    "id": "lesson-t-oop",
    "description": "Learn object-oriented programming principles for embedded firmware.",
    "content_json": [
        ...
        {"type":"text","content":"By encapsulating hardware control logic within classes, object oriented programming reduces reliance on global variables, improves code readability, and minimizes unintended side effects."},
        ...
        {"type":"text","content":"A class definition typically resides in a header file, where its interface is declared, while the implementation of its functions is placed in a corresponding cpp file. It is important to note that a class itself does not consume memory. Memory is only allocated when an object of that class is created."},
        ...
        {"type":"text","content":"Access specifiers control how other parts of the program interact with a class. Members declared as public are accessible from outside the class and define the official interface. Members declared as private are only accessible within the class itself and are used to protect internal implementation details."},
        {"type":"text","content":"For example, calling a public function such as temp.getValue() is valid because it respects the class interface. Directly accessing a private variable like temp.rawValue is not allowed and results in a compile time error."}
    ]
})

# lesson-t-rtos: Real Time Operating System (RTOS)
lessons.append({
    "id": "lesson-t-rtos",
    "description": "Understand RTOS concepts for concurrent embedded system development.",
    "content_json": [
        ...
        {"type":"text","content":"Tasks are typically structured as infinite loops that repeatedly perform a specific function. Because tasks are managed by the scheduler, they must never return. Instead of using blocking delays, tasks should call vTaskDelay() to pause execution and allow other tasks to run."},
        {"type":"text","content":"When creating tasks, care must be taken to allocate sufficient stack space to support local variables and function calls. The RTOS scheduler selects which task runs next based on priority. Lower priority values indicate less critical tasks, while higher priority values are reserved for time critical operations."},
        ...
        {"type":"text","content":"A binary semaphore acts as a simple lock or event flag. It is commonly used to signal a task when new data is available or when an interrupt has occurred."},
        {"type":"text","content":"A mutex is a specialized form of semaphore designed to protect shared resources such as displays, serial ports, or communication buses. By ensuring that only one task accesses a resource at a time, mutexes prevent data corruption."},
        {"type":"subheading","content":"Inter-Task Communication"},
        {"type":"text","content":"FreeRTOS provides several mechanisms for exchanging information between tasks: Queues allow structured data to be sent from one task to another. Semaphores provide synchronization and signaling. Task notifications offer a lightweight one-to-one signaling mechanism. Event groups allow multiple flags to represent system-wide states."},
        {"type":"subheading","content":"Tips with RTOS"},
        {"type":"bullets","content":["Keep interrupt service routines short; defer processing to tasks","Choose task priorities carefully to prevent starvation","Use vTaskDelay() instead of blocking delay()","Protect shared resources with mutexes"]},
        {"type":"text","content":"This video from DigiKey further explains Real Time Operating Systems:"},
        {"type":"youtube","content":"https://www.youtube.com/watch?v=F321087yYy4"}
    ]
})

# lesson-t-static-volatile: Static and Volatile Keywords
lessons.append({
    "id": "lesson-t-static-volatile",
    "description": "Learn the critical role of static and volatile keywords in embedded systems.",
    "content_json": [
        {"type":"text","content":"Embedded systems frequently operate under constraints such as limited memory, concurrent execution, and interaction with hardware through interrupts. In these environments, the static and volatile keywords play a critical role in ensuring that variables behave correctly and predictably."},
        ...
        {"type":"text","content":"When used on a local variable inside a function, static causes the variable to retain its value between function calls. Unlike a normal local variable, which is created on the stack and destroyed when the function returns, a static local variable is allocated once and persists for the lifetime of the program."},
        {"type":"text","content":"When applied to a global variable, static limits the variable's visibility to the source file in which it is declared. This prevents the variable from being accessed by other files and avoids unintended symbol collisions."},
        {"type":"text","content":"The static keyword can also be applied to functions. A static function is only visible within the source file where it is defined."},
        ...
        {"type":"text","content":"Without volatile, the compiler may optimize code by caching a variable's value in a register and reusing it. If a variable is modified by an interrupt, this optimization can cause the main program to miss updates, leading to incorrect or stuck behavior."},
        {"type":"text","content":"Declaring a variable as volatile forces the compiler to always read its value from memory."},
        ...
        {"type":"text","content":"This combination is often used for variables that are shared between an interrupt service routine and the main program or a task."}
    ]
})

# lesson-t-timing: Timing and Scheduling
lessons.append({
    "id": "lesson-t-timing",
    "description": "Master timing concepts for responsive and power-efficient embedded systems.",
    "content_json": [
        {"type":"text","content":"Accurate timing is a fundamental aspect of embedded system design. In a smartwatch, nearly every subsystem depends on precise and predictable timing, including screen refreshes, accelerometer interrupts, power management, step counting, button and touch input, I2C communication, battery polling, and wireless synchronization."},
        {"type":"subheading","content":"Blocking Delays"},
        {"type":"text","content":"A blocking delay halts program execution entirely for a specified period of time. During a blocking delay, the processor is unable to perform other work. This approach also wastes power because the CPU remains active while waiting."},
        {"type":"text","content":"Blocking delays are occasionally useful during early hardware bring up or simple test routines. However, they are inappropriate for production firmware in multitasking systems."},
        ...
        {"type":"text","content":"This approach enables synchronization with physical processes such as button presses, sensor updates, and display refresh cycles while allowing other parts of the system to continue running."},
        ...
        {"type":"text","content":"Unlike blocking delays, vTaskDelay() puts the calling task into a suspended state for a specified number of ticks. While the task is sleeping, the scheduler runs other tasks, allowing the system to remain responsive and power efficient."},
        ...
        {"type":"text","content":"Hardware timers are required for applications that demand extremely precise timing, such as high rate data sampling, pulse width modulation, motor control, or ultra low latency event handling."},
        ...
        {"type":"text","content":"Effective debouncing relies on accurate timing. Whether implemented in software or hardware, debounce logic uses time thresholds to distinguish between noise and valid input."}
    ]
})

# lesson-t-interrupts: Interrupts and ISRs
lessons.append({
    "id": "lesson-t-interrupts",
    "description": "Learn to handle hardware interrupts safely and efficiently.",
    "content_json": [
        ...
        {"type":"text","content":"When an interrupt occurs, the processor pauses its current execution, saves its internal state including registers and the program counter, and jumps to a predefined interrupt service routine. The interrupt service routine executes quickly and then returns control back to the original program flow."},
        {"type":"subheading","content":"IRAM_ATTR"},
        {"type":"text","content":"On the ESP32, most application code runs from external flash memory. Under certain conditions, such as flash write operations or cache misses, this memory may be temporarily unavailable. If an interrupt occurs during these periods and the interrupt handler resides in flash, the system can crash."},
        {"type":"text","content":"The IRAM_ATTR attribute places critical interrupt service routines into internal RAM instead of flash. This ensures that the interrupt handler always executes immediately and reliably. For this reason, all hardware interrupt service routines on the ESP32 should be marked with IRAM_ATTR."},
        {"type":"subheading","content":"Interrupt Trigger Types"},
        {"type":"text","content":"When configuring an interrupt, the firmware must specify what type of signal transition should trigger the interrupt. Common trigger types include detecting a transition from low to high, from high to low, or any change in signal state."},
        ...
        {"type":"text","content":"In C++ based firmware, interrupt related logic should be implemented within source files rather than directly in the main application file. This allows interrupt handling to be abstracted into classes and drivers, keeping the main program clean and readable."},
        {"type":"subheading","content":"Key Points"},
        {"type":"bullets","content":["Interrupts provide low-latency response to hardware events","ISRs must always be short and non-blocking","Use IRAM_ATTR for ESP32 interrupt handlers","Communicate via volatile flags, semaphores, or task notifications","Defer heavy processing to normal tasks"]}
    ]
})

# lesson-t-fsm: Finite State Machine (FSM)
lessons.append({
    "id": "lesson-t-fsm",
    "description": "Design structured behavior using finite state machines.",
    "content_json": [
        ...
        {"type":"text","content":"At any given time, the system exists in a single state that represents its current mode of operation. Transitions between states occur in response to events or conditions, such as a button press, a timer expiring, or new sensor data becoming available."},
        {"type":"subheading","content":"Example"},
        {"type":"text","content":"A common use of a finite state machine in smartwatch firmware is controlling the user interface. For example, the system may start in a home screen state. Each time the user presses a button or performs a gesture, the FSM transitions to the next screen state."},
        ...
        {"type":"text","content":"When implementing a finite state machine in embedded firmware, states should be represented using enumerated types to improve readability and reduce errors. The logic associated with each state should be kept isolated, either within dedicated functions or clearly defined sections of code."},
        {"type":"text","content":"State transitions can be implemented using switch statements or function pointers, depending on the complexity of the system. In firmware that uses a real time operating system, states may also be represented using task modes or shared flags."},
...
# lesson-t-networking: Networking and Wi-Fi
lessons.append({
    "id": "lesson-t-networking",
    "description": "Connect your smartwatch to Wi-Fi and synchronize time over the network.",
    "content_json": [
        {"type":"text","content":"Networking enables a microcontroller to communicate with other devices over a shared medium. The ESP32 includes an integrated Wi-Fi radio and a full TCP/IP networking stack, allowing it to function much like a small computer on a local network. This capability enables features such as time synchronization, cloud communication, remote updates, and data logging."},
        {"type":"text","content":"The ESP32 can operate in two primary Wi-Fi modes. In station mode, the device connects to an existing network such as a home router, mobile hotspot, or access point. In access point mode, the ESP32 creates its own wireless network that other devices can connect to directly. For most smartwatch applications, station mode is used."},
        ...
        {"type":"text","content":"One common use of Wi-Fi in smartwatch firmware is synchronizing the real time clock with an external time source. This is typically done using the Network Time Protocol (NTP), which allows the device to retrieve the current time from an internet time server."},
        {"type":"text","content":"After a successful Wi-Fi connection is established, the firmware can request the current time using NTP and update the real time clock accordingly. This ensures that the watch maintains accurate time even after power cycles or extended periods without user interaction."}
    ]
})

if __name__ == '__main__':
    print(f"Updating {len(lessons)} training lessons...")
    success = 0
