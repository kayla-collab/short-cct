// Script to update lessons with full verbatim content
...
// Helper to create content_json
function createContentJson(sections) {
  return JSON.stringify(sections);
}

// Execute D1 update
function updateLesson(lessonId, contentJson, description) {
  const escapedContent = escapeSql(contentJson);
  const escapedDesc = escapeSql(description);
  const cmd = `cd /home/user/webapp && npx wrangler d1 execute shortcircuits-db --remote --command="UPDATE lessons SET content_json='${escapedContent}', description='${escapedDesc}' WHERE id='${lessonId}'"`;
  try {
    execSync(cmd, { stdio: 'inherit', timeout: 30000 });
    console.log(`Updated: ${lessonId}`);
    return true;
  } catch (e) {
    console.error(`Failed: ${lessonId}`, e.message);
    return false;
  }
}
...
// Training Material - Object-Oriented Programming Basics
const lessonTOOP = {
  id: 'lesson-t-oop',
...
// Training Material - Real Time Operating System (RTOS)
const lessonTRTOS = {
  id: 'lesson-t-rtos',
  ...
  content: [
    ...
    { type: 'text', content: 'The RTOS scheduler selects which task runs next based on priority. Lower priority values indicate less critical tasks, while higher priority values are reserved for time critical operations. The highest priority level is typically used only for emergency handling or tasks triggered by interrupts.' },
...
// Training Material - Finite State Machine (FSM)
const lessonTFSM = {
  id: 'lesson-t-fsm',
...
// Training Material - Networking and Wi-Fi
const lessonTNetworking = {
...
console.log('Lesson content definitions created successfully.');
console.log('Total lessons to update:');
console.log('- Smartwatch Training: 14 lessons');
console.log('- Smartwatch Modules 1-4: 20 lessons');
console.log('- Smartwatch Module 5: 3 lessons');
console.log('- Ball & Beam Overview: 3 lessons');
console.log('- Ball & Beam Training: 7 lessons');
console.log('- Ball & Beam Controller: 6 lessons');
console.log('- Ball & Beam Final: 1 lesson');
console.log('Total: 54 lessons');
